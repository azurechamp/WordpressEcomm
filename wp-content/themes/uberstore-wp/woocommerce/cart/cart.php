...
<div class="row">
				...
				foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
					...
					if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
						...
						<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
							...
							<td class="product-name" data-title="<?php _e( 'Product', 'uberstore' ); ?>">
								<?php
									if ( ! $product_permalink ) {
									...
									if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
										echo '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'uberstore' ) . '</p>';
									}
								?>
							</td>
		
							<td class="product-price" data-title="<?php _e( 'Price', 'uberstore' ); ?>">
								<?php
									echo apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item, $cart_item_key );
								?>
							</td>
		
							<td class="product-quantity" data-title="<?php _e( 'Quantity', 'uberstore' ); ?>">
								<?php
							...
							<td class="product-subtotal" data-title="<?php _e( 'Total', 'uberstore' ); ?>">
								<?php
	...
	<aside class="sidebar woo small-12 large-4 columns cart-collaterals">
