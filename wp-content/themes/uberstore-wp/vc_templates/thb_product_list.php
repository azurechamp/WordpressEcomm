	       ...
	       $rating = $output = "";
	       	...
	       	if ( comments_open() ) {
	     	    ...
	     	    ");
	       	    ...
	       	    if ( $count > 0 ) {
	       	        ...
	       	        $rating_output = '
	       	        <div class="star-rating" title="'.sprintf(__('Rated %s out of 5', 'uberstore'), $average).'" itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">
	        ...
	        $output .= '<h5 itemprop="name"><a href="'.get_permalink($post->ID).'">'.get_the_title().'</a></h5>';
	   		...
	   		if ($product_sort == "top-rated") {
	    		...
	    		$output .= $product->get_categories( ', ', '<span class="post_categories">' . _n( '', '', $size, 'uberstore' ) . ' ', '</span>' );
